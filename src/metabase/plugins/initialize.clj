(ns metabase.plugins.initialize
  ...
  should `delay-loading`, these steps are lazily performed the first time non-trivial driver methods (such as
  connecting to a Database) are called; for all other Metabase plugins these are perfomed during launch.
...
(defmethod do-init-step! :load-namespace [{nmspace :namespace}]
  ...
  (classloader/the-classloader) ; done for side-effects to ensure context classloader is the right one
